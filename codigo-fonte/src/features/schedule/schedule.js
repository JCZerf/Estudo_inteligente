...
const testData = () => {
    ...
    doisAnosDepois.setFullYear(hoje.getFullYear() + 2);

    // Converte para datas sem hora (ISO format yyyy-mm-dd)
    const isoSelecionada = dataSelecionada.toISOString().split("T")[0];
    const isoHoje = hoje.toISOString().split("T")[0];
    const isoLimite = doisAnosDepois.toISOString().split("T")[0];

    // ⚠️ Se já confirmou a data futura, libera
    if (confirmandoDataFutura) {
        confirmandoDataFutura = false;
        return true;
    }

    // ❌ Bloqueia data anterior à hoje
    if (isoSelecionada < isoHoje) {
    ...
    // ⚠️ Pede confirmação se data for +2 anos no futuro
    if (isoSelecionada > isoLimite) {
        ...
        document.getElementById("confirmModalConfirm").onclick = () => {
            ...
            document.getElementById("eventForm").requestSubmit(); // reenvia o submit
        };
